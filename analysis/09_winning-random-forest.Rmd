...
prep_and_plot <- function(dat){
  ...
  mutate(
    ...
    ggplot() +
  geom_bar(aes(x = value, y = Metric, fill = Type), stat = 'identity') +
  scale_fill_viridis(discrete = T, option = "cividis") +
  labs(title ="", #"Accuracy Metrics for Test Dataset",
       subtitle ="", #"Random forest created with 125 trees.",
       x = "")+#"Value") +
...
facet_pair <- ggarrange(oa_plot, f1_plot, ncol=1, common.legend = T, legend = "right")
annotate_figure(facet_pair, top = text_grob("Validation Metrics for Test Dataset", face="bold", size = 14))
