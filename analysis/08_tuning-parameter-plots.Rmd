...
```{r}
current_line_plot_name <- "png_ntree_5_to_500_line_plot.png"
#current_heatmap_name <- "ntree_5_to_500_heatmap.pdf"
f1_facet_plot_name <- "png_ntree_5_to_500_facet_plot.png"
```
...
urb_dat <- dat %>%
  filter(Type == "Urban")
urb <- make_plot(urb_dat) + theme(legend.position="none") + labs(y = "\nF-1 Score\n")
nat_dat <- dat %>%
  filter(Type == "Natural")
...
legend <- get_legend(# create some space to the left of the legend
  all + theme(
    ...
    legend.box.margin = ggplot2::margin(0, 12, 0, 0)
  ))


facet_pair <-
  plot_grid(legend, urb, nat,
    nrow = 1, rel_widths = c(0.3,1,0.9)
  )
