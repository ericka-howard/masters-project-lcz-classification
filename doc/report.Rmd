...
Random forests consist of many decision trees. A decision tree can be used for classification or regression. Here I’ll focus on classification since my goal is to predict LCZ class, which is a categorical variable. Decision trees put each observation through a series of conditional statements, or splits, in order to group it with other observations that are similar. These similar groups are expected to have similar values for the predictor variable. Since the true value of the predictor variable is known for the observations in the training dataset, it’s possible to measure the accuracy of each prospective split, and therefore to build an optimal tree. 
...
$$
UA(z)\ = \frac{number\ of\ correctly\ identified\ pixels\ in\ class\ z}{total\ number\ of\ pixels\ identified\ as\ class\ z}
$$


$$
PA(z)\ = \frac{number\ of\ correctly\ identified\ pixels\ in\ class\ z}{number\ of\ pixels\ truly\ in\ class\ z}
$$
