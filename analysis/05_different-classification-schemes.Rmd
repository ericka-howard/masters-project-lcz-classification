...
library(rgdal)
library(mapproj)
source(here("R", "get_accuracy_metrics.R"))
```
...
```{r}
test2 <- test
test2$lcz_predicted <- bands_lcz_predicted
```
...
```{r}
test3 <- test
test3$lcz_predicted <- bands_lcz_predicted
```
...
```{r}
get_accuracy_metrics(test1)
get_accuracy_metrics(test2)
get_accuracy_metrics(test3)
```
...
```{r}
test4 <- test
test4$lcz_predicted <- bands_lcz_predicted
```
...
```{r}
test5 <- test
test5$lcz_predicted <- bands_lcz_predicted
```
...
```{r}
get_accuracy_metrics(test1)
get_accuracy_metrics(test4)
get_accuracy_metrics(test5)
```
